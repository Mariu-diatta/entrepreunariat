...
const VerticalMenu=()=>{
    ...
    return (
        ...
        <nav className=' sticky-top u-align-center style_css  col-lg-2'>
            ...
            <ul>
                <li>  <button className='code_u btn' onClick={()=>messageActivate?setMessageActivate(false):setMessageActivate(true)} onMouseOver={()=>setCouleurMessage(couleurBlue)} onMouseOut={()=>setCouleurMessage(couleurWhite)} style={couleurMessage}><small><i className="fa fa-commenting fa-2x" aria-hidden="true"></i>{"___"}Messages</small></button></li>
                { 
                    (messageActivate)?
