          ...
          > 
            ...
            <Offcanvas.Body>
              <Nav className="justify-content-end flex-grow-1 " style={{textAlign:'center'}}>
                <Link className='p-2  ' to="/" style={change} onMouseOver={()=>setChange(maCouleur[0])} onMouseOut={()=>setChange(maCouleur[1])} onClick={notprivatepage}><small><i className="fa fa-home fa-lg" aria-hidden="true"></i></small></Link>
                  {
                      (props.valueHeaderState!==null)?<Link style={change1} onMouseOver={()=>setChange1(maCouleur[0])} onMouseOut={()=>setChange1(maCouleur[1])} className='p-2 ' to="/admin" onClick={privatepage} ><small><i className="fa fa-user fa-lg " aria-hidden="true"></i></small></Link>:<Link style={change1} onMouseOver={()=>setChange1(maCouleur[0])} onMouseOut={()=>setChange1(maCouleur[1])} className='p-2 ' to="/inscription" ><small><i className="fa fa-lg" aria-hidden="true"><Button variant='light' className='u-btn-round'>S'inscrire</Button></i></small></Link> 
                  }
                  <Link className='p-2 ' to="/a_propos" style={change3} onMouseOver={()=>setChange3(maCouleur[0])} onMouseOut={()=>setChange3(maCouleur[1])} onClick={notprivatepage}><small><i className="fa fa-lg" aria-hidden="true"><Button variant='light' className='u-btn-round'>A propos</Button></i></small></Link>

                  {       
                      (props.valueHeaderState!==null) ?<LogoutButton onClick={logOut}/>:<Link style={change2} onMouseOver={()=>setChange2(maCouleur[0])} onMouseOut={()=>setChange2(maCouleur[1])} className='p-2' to="/login" ><i className="fa fa-lg" aria-hidden="true"><Button variant='primary' className='u-btn-round'>Se connecter</Button></i></Link>
                  }
              </Nav>
